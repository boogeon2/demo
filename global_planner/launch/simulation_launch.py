...
def generate_launch_description():
    ...
    with open(
        os.path.join(global_planner_dir, "params", "global_planner_yonsei.yaml")
    ) as global_planner_params:
        global_planner_params = yaml.load(global_planner_params, Loader=yaml.Loader)
        global_planner_params = global_planner_params["/**"]["ros__parameters"][
            "global_planner"
        ]
    ...
    cfg_file_name = (
        global_planner_dir + global_planner_params["param_files"]["cfg_file_name"]
    )
    map_file_name = (
        global_planner_dir + global_planner_params["param_files"]["map_file_name"]
    )
    graph_info_file = (
        global_planner_dir + global_planner_params["param_files"]["graph_info_file"]
    )
    guide_path_file = (
        global_planner_dir + global_planner_params["param_files"]["guide_path_file"]
    )
    ...
    robot_num_arg = DeclareLaunchArgument(
        "robot_num", default_value="1", description="Number of robots"
    )
    ...
    map_server_node = Node(
        ...
        parameters=[map_server_param, {"autostart": autostart}],
    ...
    rviz_node = Node(
        ...
        executable="rviz2",
        name="rviz2",
        arguments=["-d", os.path.join(global_planner_dir, "rviz", map_name + ".rviz")],
    ...
    local_planner_node = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(local_planner_dir, "launch", "simulation_launch.py")
        ),
        launch_arguments={
            "robot_num": robot_num,
        }.items(),
