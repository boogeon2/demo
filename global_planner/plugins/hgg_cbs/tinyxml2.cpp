...
#if defined(_MSC_VER) and (_MSC_VER >= 1400) and (!defined WINCE)
...
#if (_MSC_VER < 1400) and (!defined WINCE)
...
namespace tinyxml2
{
    ...
    char *StrPair::ParseText(char *p, const char *endTag, int strFlags, int *curLineNumPtr)
    {
        ...
        while (*p)
        {
            if (*p == endChar and strncmp(p, endTag, length) == 0)
            {
    ...
    char *StrPair::ParseName(char *p)
    {
        ...
        char *const start = p;
        ++p;
        while (*p and XMLUtil::IsNameChar(*p))
        {
            ++p;
        }
        ...
        if (_flags & NEEDS_FLUSH)
        {
            *_end = 0;
            ...
            if (_flags)
            {
                ...
                while (p < _end)
                {
                    if ((_flags & NEEDS_NEWLINE_NORMALIZATION) and *p == CR)
                    {
                    ...
                    else if ((_flags & NEEDS_NEWLINE_NORMALIZATION) and *p == LF)
                    {
                        if (*(p + 1) == CR)
                        {
                            p += 2;
                    ...
                    else if ((_flags & NEEDS_ENTITY_PROCESSING) and *p == '&')
                    {
                        ...
                        if (*(p + 1) == '#')
                        {
                            ...
                            {
                                TIXMLASSERT(0 <= len and len <= buflen);
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < NUM_ENTITIES; ++i)
                            {
                                ...
                                if (strncmp(p + 1, entity.pattern, entity.length) == 0 and *(p + entity.length + 1) == ';')
                                {
    ...
    const char *XMLUtil::ReadBOM(const char *p, bool *bom)
    {
        ...
        *bom = false;
        ...
        // Check for BOM:
        if (*(pu + 0) == TIXML_UTF_LEAD_0 and *(pu + 1) == TIXML_UTF_LEAD_1 and *(pu + 2) == TIXML_UTF_LEAD_2)
        {
            *bom = true;
            p += 3;
        }
        ...
        if (*(p + 1) == '#' and *(p + 2))
        {
            ...
            if (*(p + 2) == 'x')
            {
                ...
                while (*q != 'x')
                {
                    ...
                    TIXMLASSERT(digit == 0 or mult <= UINT_MAX / digit);
    ...
    */
    ...
    bool XMLText::ShallowEqual(const XMLNode *compare) const
    {
        ...
        return (text and XMLUtil::StringEqual(text->Value(), Value()));
    }
    ...
    bool XMLComment::ShallowEqual(const XMLNode *compare) const
    {
        ...
        return (comment and XMLUtil::StringEqual(comment->Value(), Value()));
    }
    ...
    bool XMLDeclaration::ShallowEqual(const XMLNode *compare) const
    {
        ...
        return (declaration and XMLUtil::StringEqual(declaration->Value(), Value()));
    }
    ...
    bool XMLUnknown::ShallowEqual(const XMLNode *compare) const
    {
        ...
        return (unknown and XMLUtil::StringEqual(unknown->Value(), Value()));
    }
    ...
    const char *XMLElement::Attribute(const char *name, const char *value) const
    {
        ...
        if (!value or XMLUtil::StringEqual(a->Value(), value))
        {
            return a->Value();
        }
        return 0;
    ...
    {
        if (FirstChild() and FirstChild()->ToText())
        {
    ...
    bool XMLElement::ShallowEqual(const XMLNode *compare) const
    {
        ...
        if (other and XMLUtil::StringEqual(other->Name(), Name()))
        {
    ...
    XMLError XMLDocument::LoadFile(FILE *fp)
    {
        ...
        fseek(fp, 0, SEEK_SET);
        if (fgetc(fp) == EOF and ferror(fp) != 0)
        {
    ...
    {
        TIXMLASSERT(error >= 0 and error < XML_ERROR_COUNT);
        _errorID = error;
    ...
    {
        TIXMLASSERT(errorID >= 0 and errorID < XML_ERROR_COUNT);
        const char *errorName = _errorNames[errorID];
        TIXMLASSERT(errorName and errorName[0]);
        return errorName;
    }
    ...
    void XMLDocument::PrintError() const
    {
        if (Error())
        {
            ...
            // Should check INT_MIN <= _errorID and _errorId <= INT_MAX, but that
            // causes a clang "always true" -Wtautological-constant-out-of-range-compare warning
            TIXMLASSERT(0 <= _errorID and XML_ERROR_COUNT - 1 <= INT_MAX);
    ...
    void XMLPrinter::Print(const char *format, ...)
    {
        ...
        else
        {
            ...
            TIXMLASSERT(len >= 0);
            va_start(va, format);
            TIXMLASSERT(_buffer.Size() > 0 and _buffer[_buffer.Size() - 1] == 0);
    ...
    void XMLPrinter::OpenElement(const char *name, bool compactMode)
    {
        ...
        if (_textDepth < 0 and !_firstElement and !compactMode)
        {
            Print("\n");
        }
        if (!compactMode)
    ...
    void XMLPrinter::CloseElement(bool compactMode)
    {
        ...
        {
            if (_textDepth < 0 and !compactMode)
            {
                Print("\n");
                PrintSpace(_depth);
            }
    ...
    {
        SealElementIfJustOpened();
        if (_textDepth < 0 and !_firstElement and !_compactMode)
        {
            Print("\n");
            PrintSpace(_depth);
        }
