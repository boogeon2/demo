...
bool XML_logger::get_new_log(const char *FileName)
{
    std::string value;
    ...
    value = FileName;
    value.insert(value.find_last_of("task") + 1, "_new");
...
bool XML_logger::get_replan_log(const char *FileName)
{
    std::string value;
    tinyxml2::XMLDocument doc_xml;
    ...
    value = FileName;
    value.insert(value.find_last_of("task") + 1, "_replan");
    size_t dotPos = value.find_last_of(".");
...
void XML_logger::write_to_log_path(const Solution &solution, const Map &map)
{
    ...
    for (int i = 0; i < int(solution.paths.size()); i++)
    {
        ...
        while (iter != std::prev(solution.paths[i].nodes.end()))
        {
            ...
            part->SetAttribute(CNS_TAG_GOAL_J, map.get_j(iter->id));
            if (it->g < 0)
                part->SetAttribute(CNS_TAG_ATTR_DURATION, iter->g);
            else
                part->SetAttribute(CNS_TAG_ATTR_DURATION, iter->g - it->g);
            if (it->type != -1)
            {
                part->SetAttribute("start_type", it->type);
                part->SetAttribute("goal_type", iter->type);
            }
...
void XML_logger::write_to_log_replan_path(const Solution &solution)
{
    tinyxml2::XMLElement *element = doc->FirstChildElement(CNS_TAG_ROOT);
    element = element->FirstChildElement(CNS_TAG_LOG);
    ...
    for (int i = 0; i < int(solution.paths.size()); i++)
    {
        ...
        while (iter != std::prev(solution.paths[i].nodes.end()))
        {
            part = doc->NewElement(CNS_TAG_SECTION);
            part->SetAttribute(CNS_TAG_ATTR_NUM, partnumber);
            part->SetAttribute(CNS_TAG_START_I, it->i);
            part->SetAttribute(CNS_TAG_START_J, it->j);
            iter++;
            part->SetAttribute(CNS_TAG_GOAL_I, iter->i);
            part->SetAttribute(CNS_TAG_GOAL_J, iter->j);
            if (it->g < 0)
                part->SetAttribute(CNS_TAG_ATTR_DURATION, iter->g);
            else
                part->SetAttribute(CNS_TAG_ATTR_DURATION, iter->g - it->g);
            ...
            if (it->orientation != -1)
            {
                part->SetAttribute("start_orientation", it->orientation);
                part->SetAttribute("goal_orientation", iter->orientation);
            }
            path->LinkEndChild(part);
            it++;
            partnumber++;
