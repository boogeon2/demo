...
namespace multirobot_utils
{
  ...
  namespace
  {
        ...
        const double scaled_time)
    {
      using SecF64 = std::chrono::duration<double>;
      const auto duration =
          scaled_time * std::chrono::duration_cast<SecF64>(time_range[1] - time_range[0]);
      ...
      const Time start_time, const Time finish_time) const
  {
    ...
    const Eigen::Vector3d x0 =
        multirobot_utils::compute_position(params, scaled_start_time);
    const Eigen::Vector3d x1 =
        multirobot_utils::compute_position(params, scaled_finish_time);
    const Eigen::Vector3d v0 =
        scaled_delta_t * multirobot_utils::compute_velocity(params, scaled_start_time);
    const Eigen::Vector3d v1 =
        scaled_delta_t * multirobot_utils::compute_velocity(params, scaled_finish_time);
  ...
  {
    return multirobot_utils::compute_position(
  ...
  Eigen::Vector3d Spline::compute_velocity(const Time at_time) const
  {
    ...
    return delta_t_inv * multirobot_utils::compute_velocity(
                             params, compute_scaled_time(at_time, params));
  }
  ...
  {
    const double delta_t_inv = 1.0 / params.delta_t;
    return pow(delta_t_inv, 2) * multirobot_utils::compute_acceleration(
                                     params, compute_scaled_time(at_time, params));
  }
  ...
  namespace
  {
    ...
    std::vector<double> compute_roots_in_unit_domain(const Eigen::Vector3d coeffs)
    {
      ...
      if (0.0 <= t_m and t_m <= 1.0)
        output.push_back(t_m);

      const double t_p = (-b + std::sqrt(determinate)) / (2 * a);
      if (0.0 <= t_p and t_p <= 1.0 and std::abs(t_p - t_m) > time_tolerance)
  ...
  bool DistanceDifferential::initially_approaching() const
  {
    ...
    if (derivative < 1e-8 and is_second_derivative_of_distance_negative(_params, 0.0))
      return true;
      ...
      const double eighth_1)
  {
    ...
    return (std::abs(theta - eighth_0) <= half_range) or (std::abs(theta - eighth_1) <= half_range);
  }
  ...
  std::vector<Time> DistanceDifferential::approach_times() const
  {
    ...
    for (const double t : t_vx_zero)
    {
      ...
      if (is_in_eighth(theta, M_PI / 2.0, -M_PI / 2.0) or is_negative_derivative(t, _params))
      {
    ...
    for (const double t : t_vy_zero)
    {
      ...
      if (is_in_eighth(theta, 0.0, M_PI) or is_negative_derivative(t, _params))
      {
        output.push_back(compute_real_time(_params.time_range, t));
      }
    }
    ...
    for (const double t : t_vx_p_vy)
    {
      ...
      const Eigen::Vector2d dp = compute_position(_params, t).block<2, 1>(0, 0);
      const double theta = std::atan2(dp.y(), dp.x());
      if (is_in_eighth(theta, M_PI / 4.0, -3.0 * M_PI / 4.0) or is_negative_derivative(t, _params))
      {
        output.push_back(compute_real_time(_params.time_range, t));
      }
    }
    ...
    for (const double t : t_vx_m_vy)
    {
      ...
      const Eigen::Vector2d dp = compute_position(_params, t).block<2, 1>(0, 0);
      const double theta = std::atan2(dp.y(), dp.x());
      if (is_in_eighth(theta, 3.0 * M_PI / 4.0, -M_PI / 4.0) or is_negative_derivative(t, _params))
      {
        output.push_back(compute_real_time(_params.time_range, t));
      }
    }
