...
#include "multirobot_utils/debug_Trajectory.hpp"
#include "multirobot_utils/MotionInternal.hpp"
#include "multirobot_utils/TrajectoryInternal.hpp"
...
namespace multirobot_utils
{
  ...
  public:
    ...
    InsertionResult insert(internal::WaypointElement::Data data)
    {
      ...
      if (hint != ordering.end() and hint->key == data.time)
      {
    ...
    iterator erase(iterator first, iterator last)
    {
      ...
      const auto seg_end = last._pimpl->raw_iterator == segments.end() ? segments.end() : last->_pimpl->myself;

      const auto order_start = ordering.find(seg_begin->data.time);
      const auto order_end = seg_end == segments.end() ? ordering.end() : ordering.find(seg_end->data.time);
    ...
    else
    {
      ...
      if (destination->data.time == new_time)
      {
        ...
        throw std::invalid_argument(
            "[Trajectory::Waypoint::change_time] Attempted to set time to " + std::to_string(new_time.time_since_epoch().count()) + "ns, but a waypoint already exists at that timestamp.");
        // *INDENT-ON*
      }
  ...
  void Trajectory::Waypoint::adjust_times(Duration delta_t)
  {
    ...
    if (delta_t.count() < 0 and begin_it != segments.begin())
    {
      ...
      if (new_time <= predecessor_it->data.time)
      {
        ...
        const std::string error =
            std::string("[Trajectory::Waypoint::adjust_times] ") + "The given negative change in time: " + std::to_string(delta_t.count()) + "ns caused the Waypoint's new " + "time window [" + std::to_string(tc) + "] to overlap with its precedessor's [" + std::to_string(tp) + "]";
  ...
  Trajectory::Trajectory()
      : _pimpl(multirobot_utils::make_unique_impl<Implementation>())
  {
    // Do nothing
  }
  ...
  Trajectory::Trajectory(const Trajectory &other)
      : _pimpl(multirobot_utils::make_unique_impl<Implementation>(*other._pimpl))
  {
    // Do nothing
  }
  ...
  {
    const auto &segments = _pimpl->segments;
    return segments.size() < 2 ? Duration(0) : segments.back().data.time - segments.front().data.time;
  }
...
#define DEFINE_BASIC_ITERATOR_OP(op)                           \
      ...
      const base_iterator &other) const
  {
    ...
    if (this_is_end or other_is_end)
    {
      ...
      // can be.
      return other_is_end and !this_is_end;
    }

    // If they are both valid iterators, then we can compare their times.
    return this->_pimpl->raw_iterator->data.time < other._pimpl->raw_iterator->data.time;
  }
      ...
      const base_iterator &other) const
  {
    ...
    if (this_is_end or other_is_end)
    {
      ...
      // conclusion.
      return this_is_end and !other_is_end;
    }
  ...
  Trajectory::base_iterator<SegT>::base_iterator()
      : _pimpl(multirobot_utils::make_impl<internal::TrajectoryIteratorImplementation>())
  {
    // Do nothing
  }
      ...
      const Trajectory &trajectory, const bool print_inconsistency)
  {
    ...
    for (; s_it != segments.end() and o_it != ordering.end(); ++s_it, ++o_it)
    {
    ...
    if (print_inconsistency and !consistent)
    {
      std::size_t index = 0;
      s_it = segments.begin();
      o_it = ordering.begin();
      ...
      for (; s_it != segments.end() and o_it != ordering.end();
           ++s_it, ++o_it, ++index)
      {
