...
// #include "geometry/SimplePolygon.hpp"
#include "multirobot_utils/geometry/SimplePolygon.hpp"
...
namespace multirobot_utils
{
  namespace geometry
  {
    ...
    namespace
    {
      ...
      {
        std::string output = "[multirobot_utils::Polygon] Invalid polygon "
                             "requested: " +
                             std::to_string(intersections.size()) + " pair(s) of edges intersect. See the following pairs where waypoint A "
      ...
      {
        return "[multirobot_utils::Polygon] Invalid polygon requested: " + std::to_string(num_vertices) + " vertices specified, but at least 3 " + "vertices are required for a polygon.";
      }
          ...
          const Triangle &triangle)
      {
        ...
        for (std::size_t b0 = 0; b0 < polygon.size(); ++b0)
        {
          ...
          if (b0 == a0 or b0 == a1 or b1 == a0 or b1 == a1)
            continue;
          ...
          const Triangle &triangle)
      {
        ...
        for (std::size_t i = 0; i < polygon.size(); ++i)
        {
          ...
          if (i == triangle[0] or i == triangle[1] or i == triangle[2])
            continue;
          ...
          if (s[0] + s[1] > 1.0 or s[0] < 0.0 or s[1] < 0.0)
            continue;
          ...
          const std::vector<Eigen::Vector2d> &polygon)
      {
        ...
        while (!subpolygon_queue.empty())
        {
          ...
          {
            std::cerr << "[multirobot_utils::geometry::decompose_polygon] "
    ...
    FinalShape SimplePolygon::finalize() const
    {
      ...
      return FinalShape::Implementation::make_final_shape(
          multirobot_utils::make_derived_impl<const Shape, const SimplePolygon>(*this),
