...
#include "multirobot_utils/geometry/Box.hpp"
...
// Note: The z-value doesn't really matter, as long as it's greater than 0.0
    ...
    FinalShape Box::finalize() const
    {
      double characteristic_length = 0.5 * std::sqrt(
                                               this->get_x_length() * this->get_x_length() + this->get_y_length() * this->get_y_length());
      return FinalShape::Implementation::make_final_shape(
          multirobot_utils::make_derived_impl<const Shape, const Box>(*this),
    ...
    {
      return lhs.get_x_length() == rhs.get_x_length() and
