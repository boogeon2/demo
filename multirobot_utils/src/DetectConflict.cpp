...
#include "multirobot_utils/geometry/ShapeInternal.hpp"
#include "multirobot_utils/DetectConflictInternal.hpp"
#include "multirobot_utils/ProfileInternal.hpp"
#include "multirobot_utils/Spline.hpp"
...
namespace multirobot_utils
{
  ...
  public:
        ...
        std::string function)
    {
      ...
      error._pimpl->what = std::string() + "[multirobot_utils::invalid_trajectory_error] Attempted to check a " + "conflict with a Trajectory that has [" + std::to_string(num_segments) + "] segments. This is not supported. Trajectories must have at least " + "2 segments to check them for conflicts. " + function + ":" + std::to_string(line);
      return error;
    }
    ...
    {
      invalid_trajectory_error error;
      error._pimpl->what = std::string() + "[multirobot_utils::invalid_trajectory_error] Attempting to check a " + "conflict with a Trajectory that has no shape specified for the " + "profile of its waypoint at time [" + std::to_string(time.time_since_epoch().count()) + "ns]. This is not supported.";
  ...
  invalid_trajectory_error::invalid_trajectory_error()
      : _pimpl(multirobot_utils::make_impl<Implementation>())
  {
  ...
  namespace
  {
        ...
        const double t)
    {
      ...
      return coeffs[3] * t * t * t + coeffs[2] * t * t + coeffs[1] * t + coeffs[0];
    }
    ...
    //==============================================================================
    BoundingBox get_bounding_box(const multirobot_utils::Spline &spline)
    {
    ...
    BoundingProfile get_bounding_profile(
        const multirobot_utils::Spline &spline,
        const Profile::Implementation &profile)
    {
      BoundingBox base_box = get_bounding_box(spline);

      const auto &footprint = profile.footprint;
      const auto f_box = footprint ? adjust_bounding_box(base_box, footprint->get_characteristic_length()) : void_box();

      const auto &vicinity = profile.vicinity;
      const auto v_box = vicinity ? adjust_bounding_box(base_box, vicinity->get_characteristic_length()) : void_box();
...
//==============================================================================
    ...
    //==============================================================================
    multirobot_utils::optional<double> check_collision(
        ...
        const FclContinuousCollisionRequest &request)
    {
      ...
      return multirobot_utils::nullopt;
    }
  ...
  //==============================================================================
  multirobot_utils::optional<multirobot_utils::Time> DetectConflict::between(
  ...
  //==============================================================================
  std::vector<multirobot_utils::Time> DetectConflict::custom_between( // jw 2022.11.07 added
      const Profile &profile_a,
      const Trajectory &trajectory_a,
      const Profile &profile_b,
      const Trajectory &trajectory_b,
      Interpolate interpolation)
  {
    return Implementation::custom_between(
        profile_a, trajectory_a,
        profile_b, trajectory_b,
        interpolation);
  }
  ...
  namespace
  {
    ...
    //==============================================================================
    multirobot_utils::optional<multirobot_utils::Time> detect_invasion(
        const Profile::Implementation &profile_a,
        Trajectory::const_iterator a_it,
    ...
    {
      multirobot_utils::optional<Spline> spline_a;
      multirobot_utils::optional<Spline> spline_b;
      ...
      const bool test_complement =
          (profile_a.vicinity != profile_a.footprint) or (profile_b.vicinity != profile_b.footprint);

      if (output_conflicts)
        output_conflicts->clear();

      while (a_it != a_end and b_it != b_end)
      {
        if (!spline_a)
        ...
        *motion_b = spline_b->to_fcl(start_time, finish_time);
        ...
        if (test_complement and overlap(bound_a.vicinity, bound_b.footprint))
        {
          if (const auto collision = check_collision(
        ...
        {
          spline_a = multirobot_utils::nullopt;
          ++a_it;
        }
        else if (spline_b->finish_time() < spline_a->finish_time())
        {
          spline_b = multirobot_utils::nullopt;
          ++b_it;
        }
        else
        {
    ...
    //==============================================================================
    std::vector<multirobot_utils::Time> custom_detect_invasion( // jw 2022.11.07 newly added
        const Profile::Implementation &profile_a,
        Trajectory::const_iterator a_it,
        const Trajectory::const_iterator &a_end,
        const Profile::Implementation &profile_b,
        ...
        std::vector<DetectConflict::Implementation::Conflict> *output_conflicts)
    {
      std::vector<multirobot_utils::Time> times; // store all the time of conflict
    ...
    //==============================================================================
    multirobot_utils::optional<multirobot_utils::Time> detect_approach(
        const Profile::Implementation &profile_a,
        Trajectory::const_iterator a_it,
        const Trajectory::const_iterator &a_end,
        const Profile::Implementation &profile_b,
  ...
  //==============================================================================
  multirobot_utils::optional<multirobot_utils::Time> DetectConflict::Implementation::between(
      ...
      std::vector<Conflict> *output_conflicts)
  {
    ...
    if (!profile_a.footprint and !profile_b.footprint)
      return multirobot_utils::nullopt;
    ...
    if (!profile_a.vicinity or !profile_b.vicinity)
      return multirobot_utils::nullopt;
  ...
  //==============================================================================
  std::vector<multirobot_utils::Time> DetectConflict::Implementation::custom_between( // jw 2022.11.07 added
      const Profile &input_profile_a,
      const Trajectory &trajectory_a,
      const Profile &input_profile_b,
      const Trajectory &trajectory_b,
      ...
      std::vector<Conflict> *output_conflicts)
  {
    std::vector<multirobot_utils::Time> conflicts;
    conflicts.clear();
    if (trajectory_a.size() < 2)
    {
      throw invalid_trajectory_error::Implementation ::make_segment_num_error(
    ...
    if (close_start(profile_a, a_it, profile_b, b_it))
    {
      // If the vehicles are already starting in close proximity, then we consider
      // it a conflict if they get any closer while within that proximity.
      const auto conflict = detect_approach(
          profile_a,
          std::move(a_it),
          trajectory_a.end(),
          profile_b,
  ...
  namespace internal
  {
        ...
        DetectConflict::Implementation::Conflicts *output_conflicts)
    {
      ...
      {
        std::cerr << "[multirobot_utils::internal::detect_conflicts] An invalid "
                  << "trajectory was passed to detect_conflicts. This is a bug "
                  << "that should never happen. Please alert the RMF developers."
                  << std::endl;
        throw invalid_trajectory_error::Implementation ::make_segment_num_error(trajectory.size(), __LINE__, __FUNCTION__);
      }
#endif // NDEBUG
      ...
      const Time start_time = region.lower_time_bound ? std::max(*region.lower_time_bound, trajectory_start_time) : trajectory_start_time;

      const Time finish_time = region.upper_time_bound ? std::min(*region.upper_time_bound, trajectory_finish_time) : trajectory_finish_time;
      ...
      const Trajectory::const_iterator begin_it =
          trajectory_start_time < start_time ? trajectory.find(start_time) : ++trajectory.begin();

      const Trajectory::const_iterator end_it =
          finish_time < trajectory_finish_time ? ++trajectory.find(finish_time) : trajectory.end();
      ...
      for (auto it = begin_it; it != end_it; ++it)
      {
        ...
        *motion_trajectory = spline_trajectory.to_fcl(
        ...
        assert(region.shape);
        const auto &region_shapes = geometry::FinalShape::Implementation ::get_collisions(*region.shape);
